...
import { showBlackFlashPopup } from "./jj/popup.mjs";
...
import { checkFulgorNegro, applyFulgorZonaEffect, setupFulgorNegro } from "./jj/fulgor-negro.mjs";
import { applySeiOlhosEffects } from "./jj/seis-olhos.mjs";
// executor.mjs e fulgor-negro.mjs registram seus próprios Hooks no import
    ...
    await this.actor.update({
      ...
      ...(abilityDef?.repeatable ? {
        [`system.manipulation.abilities.${abilityId}.count`]:
          (this.actor.system.manipulation?.abilities?.[abilityId]?.count ?? 0) + 1
      } : {}),
  ...
  async _onTrainAbility(trainingId, instant) {
    ...
    if ( instant ) {
      ...
      if ( (rank + 1) >= (def.maxRank ?? 3) ) {
        showBlackFlashPopup("systems/jujutsu-system/assets/satoru.gif", 2500);
      }
